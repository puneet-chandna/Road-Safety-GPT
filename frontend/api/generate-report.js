...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!userInput || userInput.trim().length < 10) {
      return res
        .status(400)
        .json({ error: "Please provide a more detailed description." });
    }
    ...
    const keywords = lowerUserInput
      .split(/\s+/)
      .filter((word) => word.length > 3);
    const relevantDocs = db.filter((doc) => {
      const searchableText =
        `${doc.problem} ${doc.category} ${doc.type}`.toLowerCase();
    ...
    if (relevantDocs.length === 0) {
      return res.status(200).json({
        error:
          "No relevant interventions found in the database. Please rephrase.",
      });
    }
...
USER'S PROBLEM: "${userInput}"`;
    ...
    res
      .status(500)
      .json({ error: "An internal server error occurred on the backend." });
  }
}
